...
| `cash` | `double` | $ | settlement (§4 of `GAME_SPECS.md`) | **Admin** + **owner** (the players that control this company) | yes | Company bank balance. May be **negative**: a shortfall is an implicit bank loan (`GAME_SPECS.md` §5). Bounded below by `-tuning.game.loan.creditLimit` per `GAME_SPECS.md` §9 invariant 2; a `null` credit limit means unlimited. Settles on every executed buy/sell of another company's shares. |
