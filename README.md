...
| Game rules | Player starts owning 100 % of a starter company; every transaction is executed by a player-controlled company; control = ≥ 20 % shares directly or transitively (Wall Street Raider-style); companies hold their own cash and stock-holding ledgers; cash may go negative as an implicit bank loan; a player is only knocked out by **bankruptcy** (net worth < 0 with no remaining shares to liquidate), not by losing control of all companies. Full rules in [`GAME_SPECS.md`](./GAME_SPECS.md); other files must conform. |
